...
class Trainer:
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc='Validation'):
                ...
                target_class = water_map.squeeze(1)
                
                accuracy = (pred_class == target_class).float().mean()
                total_accuracy += accuracy.item()
                
                # IoU for water class
                intersection = ((pred_class == 1) & (target_class == 1)).sum().float()
                union = ((pred_class == 1) | (target_class == 1)).sum().float()
                iou = intersection / (union + 1e-8)
                total_iou += iou.item()
        
        # Average metrics
        avg_loss = total_loss / len(val_loader)
        avg_frac_loss = total_frac_loss / len(val_loader)
        avg_srm_loss = total_srm_loss / len(val_loader)
        avg_accuracy = total_accuracy / len(val_loader)
        avg_iou = total_iou / len(val_loader)
        
        return avg_loss, avg_frac_loss, avg_srm_loss, avg_accuracy, avg_iou
    ...
    def train(self, train_loader, val_loader):
        ...
        for epoch in range(self.current_epoch, self.config['epochs']):
            ...
            # Validate
            val_loss, val_frac_loss, val_srm_loss, val_acc, val_iou = self.validate(val_loader)
